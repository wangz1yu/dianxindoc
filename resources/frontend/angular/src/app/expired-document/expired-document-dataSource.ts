...
export class ExpiredDocumentDataSource implements DataSource<DocumentInfo> {
  ...
  constructor(private expireDocumentService: ExpireDocumentService) {}
  ...
  loadDocuments(documentResource: DocumentResource) {
    ...
    this.expireDocumentService
      .getExpireDocuments(documentResource)
      .pipe(
        catchError(() => of([])),
